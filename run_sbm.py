...
def main():
    ...
    correlation_perc = 0.25
    for self_con in np.arange(step, max_con + (step * 0.9), step):
        ...
        cf = cost_function.CostFunction(network, pairs_reduce=0.05, ranking_weights=[np.power(i, 2) for i in reversed(range(network.num_vertices()))], verbose=0)
        mover = moves.MoveTravelSM(verbose=0)
        all_nodes = range(network.num_vertices())
        random.shuffle(all_nodes)
        print 'optimizing...'
        opt = optimizer.SimulatedAnnealing(cf, mover, all_nodes, known=0.1, max_runs=1000, reduce_step_after_fails=0, reduce_step_after_accepts=100, verbose=0)
