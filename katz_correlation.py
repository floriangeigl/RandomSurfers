...
def do_calc(i, blockp, plot):
    ...
    if i < len(blockp):
        ...
        if plot:
            pos = gt.draw.sfdp_layout(g)
            deg = g.degree_property_map("total")
            gt.draw.graph_draw(g, pos=pos, vertex_fill_color=deg, vcmap=matplotlib.cm.gist_heat_r, output=dir+"power-law-blocks.png")
    ...
    alphas = [0.0, alpha_max * 1e-8, alpha_max * 1e-7, alpha_max * 1e-6, alpha_max * 1e-5, alpha_max * 1e-4, alpha_max * 1e-3, alpha_max * 1e-2,
              alpha_max * 1e-1, alpha_max * 0.15, alpha_max * 0.2, alpha_max * 0.25, alpha_max * 0.3, alpha_max * 0.35, alpha_max * 0.4,
              alpha_max * 0.45, alpha_max * 0.5, alpha_max * 0.55, alpha_max * 0.6, alpha_max * 0.65, alpha_max * 0.7, alpha_max * 0.75,
              alpha_max * 0.8, alpha_max * 0.85, alpha_max * 0.9, alpha_max * 0.95, alpha_max * 0.96, alpha_max * 0.97, alpha_max * 0.98,
...
blockp = ["0.02, 0.02, 0.02, 0.02, 0.02; 0.02, 0.02, 0.02, 0.02, 0.02; 0.02, 0.02, 0.02, 0.02, 0.02; 0.02, 0.02, 0.02, 0.02, 0.02; 0.02, 0.02, 0.02, 0.02, 0.02",
...
for count in range(n_exper):
    for i in range(len(blockp) + 2):
        worker_pool.apply_async(func=do_calc, args=(i, blockp, id), callback=res_appender)
        worker_pool.close()
        if i > len(blockp):
            plot = False
        id += 1
