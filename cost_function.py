...
class CostFunction():
    def __init__(self, graph, deg_weight=0.1, cos_weight=0.9, pairs=None, pairs_reduce=None):
        ...
        self.deg = csc_matrix(self.deg.T)
        self.print_f('\tdeg matrix: type:', type(self.deg), 'shape:', self.deg.shape, 'filled elements:', self.deg.nnz / (self.deg.shape[0] * self.deg.shape[1]))
        self.print_f('\tgenerate cosine similarity matrix')
        self.src = None
        self.src_n = None
        self.cos_sim = filter(lambda x: x[2] > 0,
        ...
        self.print_f('\tcos sim matrix: type:', type(self.cos_sim), 'shape', self.cos_sim.shape, 'filled elements:',
                     self.cos_sim.nnz / (self.cos_sim.shape[0] * self.cos_sim.shape[1]))
    ...
    def calc_cost(self, known_nodes=None):
        ...
        for dest, srcs in self.pairs:
            ...
            # print self.best_next_hop[dest, :].toarray()[0][srcs]
            mask = csr_matrix(([1] * len(srcs), (range(len(srcs)), self.best_next_hop[dest, :].toarray()[0][srcs])), shape=prob.shape)
